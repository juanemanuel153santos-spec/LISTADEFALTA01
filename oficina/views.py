...
def marcar_comprado(request, item_id):
    ...
    return redirect('lista_falta')
@login_required
def exportar_falta_pdf(request):
    """Gera um PDF com a lista de peças em falta."""
    if not _usuario_aprovado(request.user):
        return HttpResponseForbidden("Sua conta ainda não foi aprovada.")
 
    itens = PecaEmFalta.objects.all()
 
    template = get_template('oficina/falta_pdf.html')
    html = template.render({'itens': itens})
 
    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = 'attachment; filename="pecas_em_falta.pdf"'
